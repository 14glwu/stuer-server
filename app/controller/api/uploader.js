  ...
  async uploadImgsForPost() {
    ...
    while ((part = await parts()) != null) {
      if (part.length) {
        ...
        const filename = nameid + path.extname(part.filename).toLocaleLowerCase();
        // 文件存在 upload/img 文件夹下
        const target = path.join(config.baseDir, 'upload/postImg', filename);
        // 生成一个文件写入 文件流
        const writeStream = fs.createWriteStream(target);
        try {
          await awaitWriteStream(part.pipe(writeStream));
        ...
        result.push(`${config.urls.stuer_url}/upload/postImg/${filename}`);
      }
    }
    ctx.helper.$successForEditor(result);
  }
}
